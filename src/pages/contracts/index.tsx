...
export function ContractListPage(props: ContractListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ContractInterface>>(
    () => `/contracts?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('proposal', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/proposals/view/${record.proposal?.id}`}>
              {record.proposal?.cover_letter}
            </Link>
          ),
        }
      : null,
