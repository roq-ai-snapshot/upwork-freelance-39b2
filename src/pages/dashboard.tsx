...
function DashboardPage() {
  ...
  const proposalStat = useSWR<PaginatedInterface<ProposalInterface>>(
    () => `/proposals`,
  ...
  const contractStat = useSWR<PaginatedInterface<ContractInterface>>(
    () => `/contracts`,
    () => getContracts({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Proposals',
      result: proposalStat?.data?.totalCount,
      isLoading: proposalStat.isLoading,
    ...
    {
      title: 'Contracts',
      result: contractStat?.data?.totalCount,
