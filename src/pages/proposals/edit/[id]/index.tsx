...
import { getProposalById, updateProposalById } from 'apiSdk/proposals';
...
function ProposalEditPage() {
  ...
  const handleSubmit = async (values: ProposalInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateProposalById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<ProposalInterface>({
    initialValues: data,
